...
(train_loader, test_loader), (train_dataset, test_dataset) = preprocessing.pre_processing('../resized_images')
data_len = len(train_dataset)
split_train, val_data = random_split(train_dataset, [int(data_len - data_len * 0.2), int(data_len * 0.2)])
y_train = np.array([y for x, y in iter(split_train)])

net.fit(split_train, y=y_train)
